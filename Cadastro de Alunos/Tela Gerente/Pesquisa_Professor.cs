...
namespace Cadastro_de_Alunos
{
    public partial class Pesquisa_Professor : Form
    {
		...
		private void Pesquisa_Professor_Load(object sender, EventArgs e)
        {
            using (SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["conn"].ConnectionString))
            {
                if (conn.State == ConnectionState.Closed)
                    conn.Open();
                using (SqlDataAdapter da = new SqlDataAdapter("SELECT nomeProfessor,regCREF,CPF,RG,dataNasc,Sexo,Logradouro,Complemento,numEndereco,Bairro,Cidade,UF,CEP,DDD1,telefone,DDD2,telefone_2,Email,ID_Professor FROM tbl_Professor", conn))
                {
                    da.Fill(dt);
		...
		{
			string selectQuery = "SELECT nomeProfessor,regCREF,CPF,RG,dataNasc,Sexo,Logradouro,Complemento,numEndereco,Bairro,Cidade,UF,CEP,DDD1,telefone,DDD2,telefone_2,Email,ID_Professor FROM tbl_Professor"; 
			SqlDataAdapter da = new SqlDataAdapter(selectQuery, connection);
            DataTable table = new DataTable();
			da.Fill(table);
			dgPesquisaProf.DataSource = table;
		}
		...
		{
			txtNomeEditar.Text = dgPesquisaProf.CurrentRow.Cells[0].Value.ToString();
			txtCrefEditar.Text = dgPesquisaProf.CurrentRow.Cells[1].Value.ToString();
			...
			txtLogradouro.Text = dgPesquisaProf.CurrentRow.Cells[6].Value.ToString();
			txtComplemento.Text = dgPesquisaProf.CurrentRow.Cells[7].Value.ToString();
			txtNumero.Text = dgPesquisaProf.CurrentRow.Cells[8].Value.ToString();
			txtBairro.Text = dgPesquisaProf.CurrentRow.Cells[9].Value.ToString();
			txtCidade.Text = dgPesquisaProf.CurrentRow.Cells[10].Value.ToString();
			cbxUF.Text = dgPesquisaProf.CurrentRow.Cells[11].Value.ToString();
			txtCEP.Text = dgPesquisaProf.CurrentRow.Cells[12].Value.ToString();
			txtDDD1.Text = dgPesquisaProf.CurrentRow.Cells[13].Value.ToString();
			txtTelefone.Text = dgPesquisaProf.CurrentRow.Cells[14].Value.ToString();
			txtDDD2.Text = dgPesquisaProf.CurrentRow.Cells[15].Value.ToString();
			txtTelefone2.Text = dgPesquisaProf.CurrentRow.Cells[16].Value.ToString();
			txtEmail.Text = dgPesquisaProf.CurrentRow.Cells[17].Value.ToString();
            txtID.Text = dgPesquisaProf.CurrentRow.Cells[18].Value.ToString();
        }
