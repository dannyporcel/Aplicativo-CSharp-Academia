...
namespace Cadastro_de_Alunos
{
    public partial class Pesquisa_Alunos : Form
    {
		...
		{
            string selectQuery = "SELECT nomeAluno,CPF,RG,dataNasc,Genero,Logradouro,numEndereco,Bairro,Cidade,UF,CEP,Complemento,telefone,telefone_2,DDD1,DDD2,Email,alSituacao,ID_Aluno from tbl_Aluno";          
       ...
       /* public void PreencheCampos()
        {
            SqlCommand comando = new SqlCommand("SELECT * FROM tbl_Aluno WHERE CPF =@CPF", connection);
            comando.Parameters.AddWithValue("@CPF",mskCPF.Text);
            connection.Open();
            SqlDataReader registro = comando.ExecuteReader();
            
            if(registro.Read())
                txtLogradouro.Text = registro["Logradouro"].ToString();
		...
		{	
			...
			else
			{
				...
				try
				{
					...
					populateDGV();
                    Form Pesquisa = new Pesquisa_Alunos();
                    Pesquisa.ShowDialog();
                    Close();
				}
				catch
        ...
        private void dgPesquisaAluno_MouseClick_1(object sender, MouseEventArgs e)
        {
            ...
            txtNumero.Text = dgPesquisaAluno.CurrentRow.Cells[6].Value.ToString();
            txtBairro.Text = dgPesquisaAluno.CurrentRow.Cells[7].Value.ToString();
            txtCidade.Text = dgPesquisaAluno.CurrentRow.Cells[8].Value.ToString();
            cbxUF.Text = dgPesquisaAluno.CurrentRow.Cells[9].Value.ToString();
            txtCEP.Text = dgPesquisaAluno.CurrentRow.Cells[10].Value.ToString();
            txtComplemento.Text = dgPesquisaAluno.CurrentRow.Cells[11].Value.ToString();
        ...
        {
            mskCPF.TextMaskFormat = MaskFormat.ExcludePromptAndLiterals;
            Form Pagamento = new Tela_Pagamento();
            Pagamento.ShowDialog();
            this.Hide();
        }

        private void dgPesquisaAluno_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
        ...
        /* private void PreencherCBDescricao()
         {
             ...
             string scom = "select * from tbl_Aluno";
